...
// ── Cooking presets (from togrill-bluetooth GrillType / Taste enums) ───────────
// Capability ids (strings) ↔ device integer codes. 0 = none/unset.
const GRILL_TYPES = {
  none: 0, beef: 1, veal: 2, lamb: 3, pork: 4, turkey: 5, chicken: 6,
  sausage: 7, fish: 8, hamburger: 9, bbq_smoke: 10, hot_smoke: 11,
  cold_smoke: 12, mark_a: 13, mark_b: 14, mark_c: 15,
};
const TASTES = {
  none: 0, rare: 1, medium_rare: 2, medium: 3, medium_well: 4, well_done: 5,
};

function grillTypeId(n) {
  return Object.keys(GRILL_TYPES).find(k => GRILL_TYPES[k] === n) ?? 'none';
}
function tasteId(n) {
  return Object.keys(TASTES).find(k => TASTES[k] === n) ?? 'none';
}
...
function _parseA0(data) {
  ...
  return {
    ...
    alarmInterval: data.length > 6 ? data[6] : null,  // minutes between repeated alarms
  };
}
...
function _parseA8(data) {
  // Layout: [0]A8 [1]probe [2]alarmType [3:5]temp1 [5:7]temp2
  //         [7:9]grillType [9:11]taste [11:13]time(seconds)
  return {
    type:      'alarm_detail',
    ...
    grillType: data.length > 8  ? ((data[7] << 8) | data[8]) : 0,
    taste:     data.length > 10 ? ((data[9] << 8) | data[10]) : 0,
...
// Set the cooking preset for a probe in one packet (sets BOTH grill type and
// taste — callers must pass the current value for whichever they aren't changing).
// PacketA303Write: [0xA3, probe, 0x03, grilltype_hi, grilltype_lo, taste_hi, taste_lo]
function encodeGrillTaste(probeIdx, grillTypeInt, tasteInt) {
  const gt = grillTypeInt & 0xFFFF;
  const ts = tasteInt & 0xFFFF;
  return wrapPayload(Buffer.from([0xA3, probeIdx, 0x03, (gt >> 8) & 0xFF, gt & 0xFF, (ts >> 8) & 0xFF, ts & 0xFF]));
}

// Set alarm behaviour. PacketA6Write: [0xA6, temperature_unit, alarm_interval]
// unit: 0=°C, 1=°F, null→0xFF (unchanged); alarm_interval in minutes, null→0xFF.
function encodeAlarmSettings(unitInt, alarmInterval) {
  const u = unitInt == null ? 0xFF : (unitInt & 0xFF);
  const a = alarmInterval == null ? 0xFF : (alarmInterval & 0xFF);
  return wrapPayload(Buffer.from([0xA6, u, a]));
}
